...
func (m *BaseAccess) ToORM(ctx context.Context) (BaseAccessORM, error) {
	...
	to.Owner = m.Owner
	if m.Zones != nil {
		to.Zones = make(pq.StringArray, len(m.Zones))
		copy(to.Zones, m.Zones)
	}
	if m.Clients != nil {
		to.Clients = make(pq.StringArray, len(m.Clients))
		copy(to.Clients, m.Clients)
	}
	if m.Shares != nil {
		to.Shares = make(pq.StringArray, len(m.Shares))
		copy(to.Shares, m.Shares)
	}
	if m.Relations != nil {
		to.Relations = make(pq.StringArray, len(m.Relations))
		copy(to.Relations, m.Relations)
	}
